...
if (isset($_GET["CallBackLeadEstado"]))				{$CallBackLeadEstado=$_GET["CallBackLeadEstado"];}
	elseif (isset($_POST["CallBackLeadEstado"]))	{$CallBackLeadEstado=$_POST["CallBackLeadEstado"];}
...
if ($ACTION == 'LogiNCamPaigns')
	...
	if ( ( (ereg("START|ALL",$shift_enforcement)) and (!ereg("OFF",$VU_agent_shift_enforcement_override)) ) or (ereg("START|ALL",$VU_agent_shift_enforcement_override)) )
		...
		if ( (strlen($LOGgroup_shiftsSQL) < 3) and ($VU_shift_override_flag < 1) )
			{
			$VDdisplayMESSAGE = "<B>ERROR: No hay cambios habilitado para su grupo de usuarios</B>\n";
			$VDloginDISPLAY=1;
			}
		else
			{
	...
	if ($show_campaign_list > 0)
		...
		{
		echo "<select size=1 name=VD_campaign id=VD_campaign onFocus=\"login_allowable_campaigns()\">\n";
		echo "<option value=\"\">-- USTED DEBE ENTRAR A LA PRIMERA TIMECLOCK --</option>\n";
		echo "</select>\n";
		}
	exit;
	}
...
if ($ACTION == 'regCLOSER')
	...
	if ( (strlen($closer_choice)<1) || (strlen($user)<1) )
		{
		$channel_live=0;
		echo "Opción Del Grupo $closer_choice No es válido\n";
		exit;
		}
	else
		{
	...
	echo "Closer In Opción Del Grupo $closer_choice se ha registrado al usuario $user\n";
	}
		...
		if (strlen($phone_number)>3)
			...
			#	$DB=1;
				if (strlen($dial_statuses)>2)
					...
					if (eregi("UP LAST LLAMADA TIME",$lead_order)){$order_stmt = "order by last_local_call_time desc, $last_order";}
					if (eregi("DOWN LAST LLAMADA TIME",$lead_order)){$order_stmt = "order by last_local_call_time, $last_order";}
...
if ($ACTION == 'alt_phone_change')
{
	...
	if ( (strlen($stage)<1) || (strlen($called_count)<1) || (strlen($lead_id)<1)  || (strlen($phone_number)<1) )
		{
		$channel_live=0;
		echo "TELÉFONO ALT NÚMERO ESTADO NOT CHANGED\n";
		...
		echo "TELÉFONO ALT NÚMERO ESTADO CHANGED\n";
		}
}
...
if ($ACTION == 'updateDISPO')
	...
	#	fclose($fp);
		...
		if (strlen($CallBackLeadEstado) > 0) {$log_dispo_choice = $CallBackLeadEstado;}
	...
	if ( ($dispo_choice == 'CBHOLD') and (strlen($CallBackDatETimE)>10) )
		...
		$comments = eregi_replace(';','',$comments);
		$comments = eregi_replace("\\\\",' ',$comments);
		$stmt="INSERT INTO vicidial_callbacks (lead_id,list_id,campaign_id,status,entry_time,callback_time,user,recipient,comments,user_group,lead_status) values('$lead_id','$list_id','$campaign','ACTIVE','$NOW_TIME','$CallBackDatETimE','$user','$recipient','$comments','$user_group','$CallBackLeadEstado');";
		if ($DB) {echo "$stmt\n";}
		$rslt=mysql_query($stmt, $link);
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00154',$user,$server_ip,$session_name,$one_mysql_log);}
		}

	### BEGIN Call Notas Logging ###
...
if ($ACTION == 'AGENTSview')
	...
	while ($agents_count > $loop_count)
		...
		if ($comments=='AgentXferViewSelect') 
			{
			$AXVSuserORDEN[$j] =	"$full_name$US$j";
			$AXVSuser[$j] =			$user;
	...
	if ($comments=='AgentXferViewSelect') 
		...
		while ($j > $k)
			{
			$order_split = explode("_",$AXVSuserORDEN[$k]);
			$i = $order_split[1];
...
if ($ACTION == 'CALLSINQUEUEview')
	...
	if (eregi('NONE',$view_calls_in_queue))
		{
		echo "Ver las llamadas en cola de personas de movilidad reducida para esta campaña\n";
		exit;
		}
	else
		{
		...
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; LLAMADA GROUP &nbsp; </font></TD>";
...
if ($ACTION == 'CALLLOGview')
	...
	if ($g > 0)
		...
		{echo "<tr bgcolor=white><td colspan=11 align=center>No calls en this day</td></tr>";}
...
if ($ACTION == 'SEARCHRESULTSview')
	...
	if ($camps_to_print > 0) 
		...
		elseif (strlen($last_name) > 0)
			...
			echo "<BR><BR>";
			echo "<a href=\"#\" onclick=\"hideDiv('SearcHResultSDisplaYBox');return false;\">Vaya Detrás</a>";
			echo "</CENTER>";
			exit;
			}
		...
		if ($counts_to_print > 0) 
			...
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; PHONE &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ESTADO &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; LAST LLAMADA &nbsp; </font></TD>";
	...
	if (strlen($lead_id) < 1)
		...
		if (strlen($callback_id) > 0)
			...
			if ($cb_to_print > 0)
				...
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>Callback Estado: &nbsp; </td><td ALIGN=left><font size=2>$row[0]</td></tr>";
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>Callback Lead Estado: &nbsp; </td><td ALIGN=left><font size=2>$row[7]</td></tr>";
				...
				echo "<tr bgcolor=white><td ALIGN=right><font size=2>Callback Comentarios:&nbsp; </td><td ALIGN=left><font size=2>$row[6]</td></tr>";
				echo "</TABLE>";
				echo "<BR>";
				$hide_dial_links++;
				}
...
if ($ACTION == 'CALLSINQUEUEgrab')
	...
	if ( (eregi('NONE',$view_calls_in_queue)) or (eregi('N',$grab_calls_in_queue)) )
		{
		echo "ERROR: Ver las llamadas en cola de personas de movilidad reducida para esta campaña\n";
		exit;
		}
	else
		{
